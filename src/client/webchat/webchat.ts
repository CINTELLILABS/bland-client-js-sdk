import { IWebchatConfiguration, IWebchatState, IWebchatWSSettings } from "../../types/webchat";
import Bland from "../client";
import Websocket from "isomorphic-ws";

class Webchat extends Bland {
    state: IWebchatState
    ...
    constructor(options: any) {
        super(options)
        this.endpoint = options?.endpoint || "wss://stream-v2.aws.dc8.bland.ai/ws/connect/blandshared"
        this.connectionUrl = null
        this.wsSettings = { interval: 15000, timeout: 10000 }
        this.state = "closed"
    }

    private params(params: Record<string, string | boolean | number | undefined>) {
    ...
    public async start(config: IWebchatConfiguration) {
        if (this.state !== "closed" && this.state !== "idle") throw new Error("Webchat already started")
        this.state = "connecting"

        if (!config.sessionId) {
            this.state = "closed"
            throw new Error("A session token is required to start the webchat. Please use the admin client server-side to generate a session token.")
        }

        if (!config.agentId) {
            this.state = "closed"
            throw new Error("An agent ID is required to start the webchat. Please create an agent in the dashboard, and use the agent ID here.")
        }
    ...
    private create() {
        if (!this.connectionUrl) {
            this.state = "closed"
            throw new Error("Connection URL is not set. Please call start() before creating the websocket connection.")
        }
    ...
    public stop() {
        if (this.websocket && (this.websocket.readyState === this.websocket.OPEN || this.websocket.readyState === this.websocket.CONNECTING)) {
            this.state = "closing"
    ...
    private keepAlive(stop?: boolean) {
        if (stop) {
            ...
            this.keepAliveInterval = setInterval(() => {
                if (this.lastActivityAt && Date.now() - this.lastActivityAt > this.wsSettings.timeout) {
                    this.stop()
                    this.state = "closed"
                }
    ...
    private teardownAudio() {
        ...
        if (this.sinkGain) {
            try { this.sinkGain.disconnect() } catch {}
            this.sinkGain = null
        }
        if (this.micStream) {
    ...
    private playPcm(u8: Uint8Array) {
        ...
        const buffer = this.audioContext.createBuffer(1, f32.length, this.audioContext.sampleRate)
